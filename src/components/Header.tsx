...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0b0f19]/80 backdrop-blur-2xl text-slate-100 border-b border-white/10 shadow-2xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-4">
        ...
        >
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shadow-[0_0_20px_rgba(99,102,241,0.2)] group-hover:scale-105 group-hover:bg-indigo-500/20 transition-all shrink-0">
          ...
          <div>
            <h1 className="text-base sm:text-lg font-bold tracking-tight text-white font-display flex items-center gap-2.5 leading-none">
              ...
              <span className="text-[10px] font-semibold px-2.5 py-0.5 rounded-full glass-pill border border-indigo-500/30 shrink-0 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-indigo-400" /> AI Studio
              </span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 shrink-0 ml-auto">
          ...
          {currentUser ? (
            <button
              onClick={() => logout()}
              title="Sign Out"
              className="p-2 text-slate-300 hover:text-red-400 hover:bg-white/10 rounded-full transition-colors min-h-[38px] min-w-[38px] flex items-center justify-center border border-white/10 glass-panel"
            >
          ...
          {!showLandingPage && (
            <button
              onClick={onNewBook}
              id="new-book-btn"
              className="inline-flex items-center gap-2 px-5 py-2 text-xs font-bold btn-primary transition-all min-h-[38px]"
            >
