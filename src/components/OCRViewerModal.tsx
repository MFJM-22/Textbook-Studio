...
export const OCRViewerModal: React.FC<OCRViewerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4">
      <div className="glass-panel bg-[#0b0f19] rounded-3xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden border border-white/10 text-slate-100">
        {/* Header Bar */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 bg-white/[0.02] text-white shrink-0 flex-wrap sm:flex-nowrap gap-2 border-b border-white/10">
          <div className="flex items-center gap-3">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/30">
              OCR Verification Studio
            </span>
            <div>
              <h2 className="font-bold text-sm sm:text-base text-white font-display truncate max-w-[200px] sm:max-w-md">
                {book.title}
              </h2>
          ...
          <div className="flex items-center gap-2 shrink-0 ml-auto">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full text-xs font-bold shadow-lg shadow-indigo-600/30 transition-all min-h-[38px] disabled:opacity-50 cursor-pointer"
            >
        ...
        {pages.length === 0 ? (
          ...
          <div className="flex-1 p-8 flex flex-col items-center justify-center text-center bg-[#0b0f19] space-y-4">
            ...
            <div className="max-w-md space-y-2">
              <h3 className="text-lg font-bold text-white font-display">No Scanned Pages Found Yet</h3>
            ...
            {isUploadingMore && (
              <div className="flex items-center gap-2 text-xs font-semibold text-indigo-400 bg-indigo-500/10 px-4 py-2 rounded-full border border-indigo-500/30 animate-pulse">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <label
                ...
                className={`cursor-pointer px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-full shadow-lg shadow-indigo-600/30 flex items-center gap-2 transition-all ${
              ...
              <button
                ...
                className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 border border-white/10 text-slate-200 font-semibold text-xs rounded-full shadow-md flex items-center gap-2 transition-all cursor-pointer"
              >
            ...
            {showAddTextInput && (
              <div className="w-full max-w-xl glass-panel bg-slate-900/90 p-4 rounded-3xl border border-white/10 text-left space-y-3 mt-4 shadow-2xl">
                <label className="block text-xs font-bold text-slate-300">Paste Note Content</label>
                <textarea
                  rows={5}
                  ...
                  className="w-full p-3 text-xs border border-white/10 bg-slate-950 rounded-2xl focus:ring-2 focus:ring-indigo-500 text-slate-100 placeholder-slate-500 font-mono"
                />
                <div className="flex justify-end gap-2">
                  <button
                    onClick={() => setShowAddTextInput(false)}
                    className="px-3 py-1.5 text-xs text-slate-400 hover:text-white hover:bg-white/10 rounded-full"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={isUploadingMore}
                    className="px-4 py-1.5 text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white rounded-full flex items-center gap-1.5 cursor-pointer"
                  >
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-white/10 overflow-hidden">
              ...
              <div className="p-4 bg-[#0b0f19] overflow-y-auto flex flex-col items-center justify-start">
                <div className="w-full mb-3 flex items-center justify-between text-xs font-semibold text-slate-300 bg-slate-900 px-3 py-2 rounded-2xl border border-white/10 shadow-2xs">
                  ...
                  <div className="flex items-center gap-1 text-slate-400">
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-white/10 hover:text-white rounded ml-1"
                      title="Reset Zoom"
                    >
                ...
                <div className="w-full flex-1 flex items-center justify-center p-2 rounded-2xl bg-black/40 border border-white/10 overflow-auto">
                  {currentPage?.image_url && currentPage.image_url.startsWith('data:image') ? (
                    <img
                      src={currentPage.image_url}
                      alt="Scanned note page"
                      style={{ transform: `scale(${zoomLevel / 100})`, transformOrigin: 'top center' }}
                      className="max-h-[60vh] object-contain rounded-xl shadow-2xl transition-transform duration-200 border border-white/10"
                    />
                  ) : (
                    <div className="p-6 bg-slate-900 rounded-2xl border border-white/10 text-center space-y-2 max-w-sm">
                      <FileText className="w-10 h-10 text-indigo-400 mx-auto" />
                      <h4 className="text-xs font-bold text-white">Typed Raw Note Page</h4>
              ...
              <div className="p-4 flex flex-col bg-[#0b0f19] overflow-y-auto space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-white/10">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={isReOCRing}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-300 bg-slate-900 hover:bg-slate-800 border border-white/10 rounded-full transition-colors cursor-pointer"
                      title="Re-run Gemini Vision OCR on this page"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isReOCRing ? 'animate-spin text-indigo-400' : ''}`} />
                ...
                <textarea
                  ...
                  className="flex-1 w-full p-3.5 font-mono text-xs border border-white/10 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-900 text-slate-100 placeholder:text-slate-500 resize-none leading-relaxed shadow-inner min-h-[260px]"
                />

                <div className="p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl text-[11px] text-indigo-300 flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
                  <div>
                    <strong>Formatting Tip:</strong> Confirm that markdown tables format like{' '}
                    <code className="bg-indigo-950 px-1 py-0.5 rounded text-[10px] text-indigo-200">
                      | Header 1 | Header 2 |
                    </code>{' '}
